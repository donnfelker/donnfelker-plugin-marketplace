...
- Bumped `durable-spec-development` to v1.3.0: added the `pr-autopilot` skill — point it at a single PR and it loops unattended (fetch unaddressed reviews/comments → address actionable ones → commit, push, reply, resolve threads → re-request review from the bots whose change requests it addressed) until a quiet round or a `--max-rounds` / `--time-cap` safety cap, then prints a results summary. Factored the plugin-wide single-PR review mechanics (three-source fetch, classify, reply/resolve, per-bot re-review) into `references/pr-review-mechanics.md`, shared by `pr-autopilot`, `address-pr-comments`, and `implement-full-spec`'s Phase C.
