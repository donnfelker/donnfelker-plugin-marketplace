...
name: pr-autopilot
description: >
  Autonomously watch a single pull request and drive its review feedback to completion without
  supervision. Each round it fetches every unaddressed review and comment (from Claude, CodeRabbit,
  Copilot, other bots, and humans), addresses the actionable ones, commits and pushes a fix,
  replies/resolves the threads, and re-requests review from the bots whose change requests it
  addressed — then loops, waiting for new reviews to arrive, until a quiet round (or a safety cap) is
  reached, at which point it stops and prints a results summary. Use whenever the user wants to "watch
  a PR", "babysit a PR", "keep addressing PR comments until they're done", "auto-respond to reviews",
  "handle the review back-and-forth on PR #N", or run a hands-off review-response loop on ONE PR —
  even if they don't name this skill. Unlike /address-pr-comments (one pass, asks before committing),
  this runs unattended across multiple rounds and commits/pushes on its own. For driving MANY stacked
  PRs from a parent ticket, use /implement-full-spec instead.
user_invocable: true
---
